...
signupRouter.route('/')
  ...
  .post(validationSchema(signupAuthenticationSchema, 'body', undefined, false), catchHandlerController(signupController.sendOTP))

signupRouter.route('/confirmation')
  // For check the received OTP code and signup the user in DB if ok
  .post(validationSchema(signupAuthenticationSchema, 'body', undefined, false), catchHandlerController(signupController.checkUserByOTP));
