import logObject from "sockjs-client/lib/utils/log.js";
...
const activityController = {
    async index(req, res) {
        try {
            ...
            const activitiesOfCity = await dataMapper.findActivityOfCity(country, city);

            if (!activitiesOfCity || activitiesOfCity.length === 0) {
                return res.status(404).json({ message: `No activities found for ${city} and ${country}` });
            }
    ...
    async show(req, res) {
        ...
        console.log(id);
            ...
            if (!activity) {
                return res.status(404).json({ message: `No activity found for id ${id}` });
            }
    ...
    async showRecent(req, res) {
        try {
            ...
            console.error('Error in activityController: showRecent method', error.message);
            res.status(500).json({ error: 'An error occurred' });
        }
    },
    ...
    async store(req, res) {
        ...
        const { slug, url, title, description, url_image, address, phone, avg_rating,latitude, longitude, id_user, id_city, cityname } = req.body;
